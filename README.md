...
```
index.html      página única (HTML/CSS/JS, sem dependências externas além das fontes Google)
dados.js        registro de ocorrências, comparativo de relatórios e cláusulas
evidencias/     12 imagens tratadas para proteção de dados
```

## Proteção de dados

Documento de uso interno e restrito às tratativas contratuais. Antes da publicação foram
**omitidos ou tarjados**: nomes completos das colaboradoras, CPF, RG, números de telefone e
rostos. O book cadastral das degustadoras (`AGRICOLA FAMOSA (1).pdf`) **não é reproduzido**.
A página está marcada com `noindex,nofollow`.

Para substituir os primeiros nomes remanescentes por códigos (D1…D11), edite `dados.js`.

---
75 LAB Estratégia, Design e Produção · emissão 01/09/2026
